class Plant {
  private static readonly MAX_GROW_LIGHTLEVEL: int = 9;
  private static readonly GROW_PARTICLES = 37;
  ...
  public static growCustom(
    ...
    return (
      World.setBlock(coords.x, coords.y, coords.z, block.id, data),
      ...
      (coords, item, block, player) => {
        ...
        return (
          ...
          Plant.growCustom(
            ...
            random < this.stages
              ? random
              : block.data + Math.abs(block.data - random),
            coords,
            item,
            block,
            player,
